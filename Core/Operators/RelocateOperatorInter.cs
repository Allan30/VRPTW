...
public class RelocateOperatorInter : OperatorInter
{
    ...
    protected override double CheckDelta(LinkedListNode<Client> nodeSrc, LinkedListNode<Client> nodeTrg, Vehicle vehicleSrc, Vehicle vehicleTrg)
    {
        ...
        if (delta > Delta || !vehicleSrc.StayCorrectWindow(newTimeSrc, -nodeSrc.Value.Demand, nodeSrc.Next, vehicleSrc.Clients.Last) || !vehicleTrg.StayCorrectWindow(newTimeTrg, nodeSrc.Value.Demand, nodeTrg, vehicleTrg.Clients.Last))
        {
            return double.NaN;
        }
